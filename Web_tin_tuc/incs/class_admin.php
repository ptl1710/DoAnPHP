...
class adminlib extends dblib
{
	...
	function add_category()
	{
		...
		if (empty($data['name'])) {
			$error['name'] = 'Bạn chưa nhập tên chuyên mục';
		}

		if (!$error) {
			$this->insert("category", $data);
			$error["note"] = "Thêm chuyên mục thành công";
			header('Location:category.php');
			die();
		} else {
			$error["note"] = "Thêm chuyên mục thất bại";
		}
	...
	function remove_category($category_id)
	{
		...
		$this->get_row($sql);
		$this->remove("category", "category_id = $category_id");
		header('Location:category.php');
		die();
	}
	function update_category($category_id)
	{
		...
		if (empty($data['name'])) {
			$error['name'] = 'Bạn chưa nhập chuyên đề';
		}

		if (!$error) {

			$this->update("category", $data, "category_id = $category_id");
			$error["note"] = "Cập nhật chuyên đề thành công";
		} else {
			$error["note"] = "Cập nhật bài viết thất bại";
		}
